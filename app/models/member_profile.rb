class MemberProfile < ApplicationRecord
  ...
  def self.profile_timeline(data, current_user)
    begin
      ...
      if posts.present?
        Post.where("created_at > ? AND member_profile_id = ?", posts.first.created_at, profile.id).present? ? previous_page_exist = true : previous_page_exist = false
        Post.where("created_at < ? AND member_profile_id = ?", posts.last.created_at,  profile.id).present? ? next_page_exist = true : next_page_exist = false
      end
